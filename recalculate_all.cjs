...
async function main() {
  ...
  try {
    ...
    // Let's generate a sequence of dates from 2026-05-15 to 2026-06-25
    const dates = [];
    ...
    console.log(`Recalculating for ${dates.length} days...`);

    for (const b of branches) {
      if (b.id === 1) continue; // Skip Head Office or handle if needed, usually skipped for branches
      ...
      for (const dStr of dates) {
        ...
        // If there is activity, or if there is already a row, we write to daily_cash_balances
    ...
    console.log("\nRecalculation successfully finished!");
