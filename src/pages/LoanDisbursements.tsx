    ...
    <div className="space-y-8 pb-10">
          ...
          >
            <div className="p-6 md:p-8 flex flex-col items-center text-center">
               <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 shadow-inner ${
                  confirmAction.status === 'active' ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'
               }`}>
                 {confirmAction.status === 'active' ? <Banknote className="w-8 h-8" /> : <X className="w-8 h-8" />}
               </div>
              ...
              <div className="flex flex-col sm:flex-row w-full gap-3 sm:gap-4">
                ...
                <button
                  ...
                  className={`flex-1 py-3.5 rounded-2xl text-sm font-black text-white hover:opacity-90 active:scale-[0.98] transition-all shadow-lg uppercase tracking-widest disabled:opacity-50 flex items-center justify-center ${
                     confirmAction.status === 'active' ? 'bg-emerald-500 shadow-emerald-500/30' : 'bg-rose-500 shadow-rose-500/30'
                  }`}
                >
      ...
      <div className="bg-white rounded-[40px] p-4 sm:p-10 shadow-xl shadow-indigo-500/5 border border-slate-100 overflow-hidden">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10 border-b border-slate-100 pb-8 px-2 sm:px-0">
          <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">Loan Disbursements</h1>
            <p className="text-slate-500 font-medium text-sm mt-0.5 uppercase tracking-widest leading-none">Review and disburse approved loan requests</p>
          </div>
          <div className="flex items-center gap-4">
            <span className="bg-emerald-100 text-emerald-800 font-bold px-4 py-2 rounded-xl text-sm border border-emerald-200">
              {filteredLoans.length} Approved
            </span>
            ...
            <table className="w-full border-collapse min-w-[1000px]">
              <thead>
                <tr className="bg-gradient-to-r from-emerald-500 to-teal-500 text-white">
                  <th className="text-left text-white text-[11px] font-black uppercase tracking-[0.2em] p-6 border-b border-emerald-500/20 rounded-tl-xl">Borrower / Branch</th>
                  <th className="text-left text-white text-[11px] font-black uppercase tracking-[0.2em] p-6 border-b border-emerald-500/20">Product</th>
                  <th className="text-left text-white text-[11px] font-black uppercase tracking-[0.2em] p-6 border-b border-emerald-500/20">Principal</th>
                  <th className="text-left text-white text-[11px] font-black uppercase tracking-[0.2em] p-6 border-b border-emerald-500/20">Terms</th>
                  <th className="text-left text-white text-[11px] font-black uppercase tracking-[0.2em] p-6 border-b border-emerald-500/20">Cycle EMI</th>
                  <th className="text-left text-white text-[11px] font-black uppercase tracking-[0.2em] p-6 border-b border-emerald-500/20">Apply Date</th>
                  <th className="text-right text-white text-[11px] font-black uppercase tracking-[0.2em] p-6 border-b border-emerald-500/20 rounded-tr-xl">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filteredLoans.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-slate-400 font-bold uppercase tracking-widest text-xs">No approved applications awaiting disbursement</td>
                  </tr>
                ) : (
                    ...
                    <tr key={loan.id} className="group hover:bg-slate-50 transition-all duration-300">
                      <td className="p-6">
                        <div className="flex flex-col">
                          <span className="text-[15px] text-slate-900 font-black tracking-tight group-hover:text-emerald-600 transition-colors uppercase">{loan.member_name}</span>
                          <div className="flex items-center gap-1.5 mt-1">
                            <span className="bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded text-[10px] font-bold tracking-widest">{loan.loan_no || `L${loan.id}`}</span>
                          ...
                          {loan.scheme_name && <span className="text-[10px] text-emerald-600 font-black uppercase tracking-widest mt-1 opacity-70">{loan.scheme_name}</span>}
                          {loan.emi_frequency === 'monthly' && loan.collection_week && (
                            <span className="text-[9px] font-black text-emerald-500 uppercase tracking-tighter mt-0.5 flex items-center gap-1">
                      ...
                      <td className="p-6">
                        <span className="text-[11px] text-emerald-600 font-black uppercase tracking-widest bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100">{loan.duration_weeks} WKS</span>
                      </td>
                      <td className="p-6">
                  ...
                  <div className="bg-gradient-to-r from-emerald-500 to-teal-500 p-4 text-white flex justify-between items-center">
                    ...
                    {loan.scheme_name && (
                      <div className="flex flex-col items-end">
                        ...
                        {loan.emi_frequency === 'monthly' && loan.collection_week && (
                          <span className="text-[8px] font-black uppercase tracking-tighter bg-emerald-400 text-white px-1.5 py-0.5 rounded mt-1 shadow-sm">
                            {loan.collection_week}
                          </span>
                        )}
                      </div>
